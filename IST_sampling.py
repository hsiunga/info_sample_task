...
def create_trial_buttons(win, majority_cat):
    sample_button = visual.TextStim(win, text=u"Sample", pos=(0, 0), bold=True)
    xdelta = button_position()
    if majority_cat == 'indoor':
        maj_button = visual.TextStim(win, text=u"Indoor", pos=(xdelta, 0), bold=True)
        min_button = visual.TextStim(win, text=u"Outdoor", pos=(-xdelta, 0), bold=True)
    elif majority_cat == 'outdoor':
        maj_button = visual.TextStim(win, text=u"Outdoor", pos=(xdelta, 0), bold=True)
        min_button = visual.TextStim(win, text=u"Indoor", pos=(-xdelta, 0), bold=True)
    elif majority_cat == 'nonliving':
        maj_button = visual.TextStim(win, text=u"Non-Living", pos=(xdelta, 0), bold=True)
        min_button = visual.TextStim(win, text=u"Living", pos=(-xdelta, 0), bold=True)
    else:
        maj_button = visual.TextStim(win, text=u"Living", pos=(xdelta, 0), bold=True)
        min_button = visual.TextStim(win, text=u"Non-Living", pos=(-xdelta, 0), bold=True)
    if xdelta < 0:
        majority_side = 'left'
    else:
        majority_side = 'right'
...
def main():
    ...
    filename = config.config.get('log_location') + os.sep + '{}_{}_{}_IST_sampling'.format(participant.id, participant.session,data.getDateStr()) + '.csv'
    ...
    # set up directions
    judge_ioc = visual.TextStim(win, text=u"indoor/outdoor", pos=(0, 0), bold=True)
    judge_lnc = visual.TextStim(win, text=u"living/nonliving", pos=(0, 0), bold=True)
    ...
    for no, trial in enumerate(trial_types_idx):
        ...
        trial_clock.reset()
        start_screen(win, trial_data.get('category_of_pic'), trial_data.get('reward_type'), wait=1.5)
        sample_button, maj_button, min_button, trial_object.majority_side = create_trial_buttons(win, trial_object.majority_cat)
        next_unseen_pic = 0
        #print(trial_pics)
